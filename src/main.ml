...
in
let unknow arg =
  if !source_file = None then source_file := Some arg
  else error (Printf.sprintf "don't know what to do with: `%s'" arg)
in
Arg.parse spec unknow usage;
let print_msg = if !verbose then Printf.printf "%s%!" else fun _ -> () in
...
let compile loadfn exportfn =
  try
    ...
    let rec compress_loop orig_code prim data =
      ...
      print_done ();
      if orig_code = code then (code, data) else compress_loop code prim data
    in
    ...
    print_msg
      (Printf.sprintf
         ...
         orig_instr_nb new_instr_nb
         (gain orig_instr_nb new_instr_nb)
         orig_code_length new_code_length
         (gain orig_code_length new_code_length)
         orig_data_nb new_data_nb
         (gain orig_data_nb new_data_nb)
         orig_data_length new_data_length
         (gain orig_data_length new_data_length)
         orig_prim_nb new_prim_nb
         (gain orig_prim_nb new_prim_nb)
         orig_prim_length new_prim_length
         (gain orig_prim_length new_prim_length)
         orig_file_length new_file_length
         (gain orig_file_length new_file_length))
  with
  | Sys_error msg | Index.Exn msg -> error msg
  | Code.Exn msg | Instr.Exn msg | Data.Exn msg | Dump.Exn msg | Prim.Exn msg ->
      Printf.eprintf "Error: %s\n" msg;
      exit 1
in
match Filename.check_suffix !dest_file ".c" with
