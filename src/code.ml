...
let compute_ptrs code =
  ...
  let affect_ptr instr =
    ...
    | Switch (_, tab) -> Array.iter (update_pointed 2) tab
    | _ -> ()
  in
  for i = pred nb_instr downto 0 do
    grep_instr i code.(i)
  done;
...
let parse ic index =
  ...
  let read =
    ...
    fun () ->
      ...
      | ws ->
          raise
            (Exn (Printf.sprintf "Unsupported architecture: word size is %d" ws))
  in
  let rec f i acc =
    let old_addr = !cpt in
    ...
    | None -> acc
    | Some bcs ->
        let instrs = List.map (fun bc -> { old_addr; bc }) bcs in
  ...
  compute_ptrs code;
  Array.map (fun { bc; old_addr = _ } -> bc) code
...
let parse_c ic =
  ...
  in
  let opcodes =
    scan_fold "static int caml_code[] = {" "0x8f};"
      (fun acc line ->
        let bits = Str.split (Str.regexp_string ", ") line in
        let opcodes =
          List.map
            (fun s ->
              Scanf.sscanf s "0x%02x%02x%02x%02x" (fun c3 c2 c1 c0 ->
                  ...
                  | ws ->
                      raise
                        (Exn
                           (Printf.sprintf
                              "Unsupported architecture: word size is %d" ws))))
            bits
        in
        acc @ opcodes)
      []
  ...
  compute_ptrs code;
  let ops = Array.map (fun { bc; old_addr = _ } -> bc) code in
